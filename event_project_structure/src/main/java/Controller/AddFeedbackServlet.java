...
public class AddFeedbackServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    request.getRequestDispatcher("Pages/AddFeedback.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		try 
		{
			...
			boolean result = feedbackService.insertFeedback(feedback);
			if(result) 
			{
				request.setAttribute("message", "added feedback");
				List<Feedback> feedbackList = feedbackService.getFeedbackByAttendeeId(attendeeId);
