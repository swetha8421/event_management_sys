	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String password = request.getParameter("password");
		 if (!email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
	            response.sendRedirect("Pages/attendeeLogin.jsp?error=invalidEmail");
	            return;
	        }
		...
		Attendee result = attendeeLoginService.loginByEmailRole(email);
		...
		if(result == null || !result.getPassword().equals(password))
		{
			...
			response.sendRedirect("Pages/attendeeLogin.jsp?error=invalid");
			return;
		...
		response.sendRedirect("Pages/attendeeHomePage.jsp");
